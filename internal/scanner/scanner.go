...
// It mirrors provisioner.DiskEntry but lives here to avoid a circular import.
...
func (s *Scanner) scan(ctx context.Context) error {
	...
	diskMap := make(map[string]DiskEntry, len(mounted))
	currentUUIDs := make(map[string]struct{}, len(mounted))
	for _, dev := range mounted {
		if dev.UUID == "" || dev.UUID == "N/A" {
			continue
		}
		diskMap[dev.UUID] = DiskEntry{
			Path: dev.MountPath,
			Size: dev.TotalSize,
		}
...
func (s *Scanner) patchNode(ctx context.Context, diskMap map[string]DiskEntry, currentUUIDs map[string]struct{}) error {
	...
	for uuid := range s.prevUUIDs {
		if _, ok := currentUUIDs[uuid]; !ok {
			labels[common.LabelUUIDPrefix+uuid] = nil
		}
	}
