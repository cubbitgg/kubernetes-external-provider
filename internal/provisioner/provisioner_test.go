...
func TestProvision_Success(t *testing.T) {
	diskMap := map[string]provisioner.DiskEntry{
	...
	pvc := &corev1.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{
			...
			Namespace: "default",
			Annotations: map[string]string{
				common.PVCAnnotationUUID: "test-uuid-1234",
			},
		},
	...
	if pv.Annotations[common.PVCAnnotationUUID] != "test-uuid-1234" {
	...
	_, state, err := p.Provision(context.Background(), controller.ProvisionOptions{
		...
		PVC: &corev1.PersistentVolumeClaim{
			ObjectMeta: metav1.ObjectMeta{
				...
				Namespace: "default",
				Annotations: map[string]string{
					common.PVCAnnotationUUID: "wanted-uuid",
				},
			},
		},
		SelectedNodeName: "worker-1",
...
func TestDelete_ForeignPV(t *testing.T) {
	...
	pv := &corev1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			Name: "pv-foreign",
			Annotations: map[string]string{
				common.AnnotationProvisionedBy: "other.provisioner.io",
			},
		},
	}
