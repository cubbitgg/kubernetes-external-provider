...
func (p *LocalDiskProvisioner) Provision(ctx context.Context, options controller.ProvisionOptions) (*corev1.PersistentVolume, controller.ProvisioningState, error) {
	...
	uuid := options.PVC.Annotations[commonlib.PVCAnnotationUUID]
	if uuid == "" {
		return nil, controller.ProvisioningFinished, fmt.Errorf(
			"PVC %s/%s missing required annotation %s",
			options.PVC.Namespace, options.PVC.Name, commonlib.PVCAnnotationUUID,
		)
	}
	...
	diskMapJSON := node.Annotations[commonlib.AnnotationDiskMap]
	if diskMapJSON == "" {
		...
		return nil, controller.ProvisioningReschedule, fmt.Errorf(
			...
			nodeName, commonlib.AnnotationDiskMap,
		)
	}

	var diskMap map[string]commonlib.DiskEntry
	...
	pv := &corev1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				commonlib.PVCAnnotationUUID:       uuid,
			},
		},
...
func (p *LocalDiskProvisioner) Delete(_ context.Context, pv *corev1.PersistentVolume) error {
	if pv.Annotations[commonlib.AnnotationProvisionedBy] != commonlib.ProvisionerName {
		return &controller.IgnoredError{
			Reason: fmt.Sprintf("PV %s was not provisioned by %s", pv.Name, commonlib.ProvisionerName),
		}
	}
